...
from currencycloud.resources import WithdrawalAccount, WithdrawalAccountFunds
...
class TestWithdrawalAccounts:
    ...
    async def test_can_find_withdrawal_account(self) -> None:
        with my_vcr.use_cassette("withdrawal_accounts/can_find_withdrawal_account.json"):
            withdrawal_accounts = await self.client.withdrawal_accounts.find(
            ...
            assert (
                withdrawal_account.account_id == "72970a7c-7921-431c-b95f-3438724ba16f"
            )

    async def test_can_find_withdrawal_account2(self) -> None:
        with my_vcr.use_cassette("withdrawal_accounts/can_find_withdrawal_account2.json"):
            withdrawal_accounts = await self.client.withdrawal_accounts.find()
            ...
            assert (
                withdrawal_account.account_id == "72970a7c-7921-431c-b95f-3438724ba16e"
            )

    async def test_can_pull_funds(self) -> None:
        with my_vcr.use_cassette("withdrawal_accounts/can_pull_funds.json"):
            withdrawal_account_funds = await self.client.withdrawal_accounts.pull_funds(
            ...
            assert (
                withdrawal_account_funds.withdrawal_account_id
                == "0886ac00-6ab6-41a6-b0e1-8d3faf2e0de2"
            )
