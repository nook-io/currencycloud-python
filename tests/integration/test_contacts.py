...
class TestContacts:
    ...
    async def test_contacts_can_get_current(self) -> None:
        with my_vcr.use_cassette("contacts/can_get_current.json"):
            contact = await self.client.contacts.current()
    ...
    async def test_contacts_can_create(self) -> None:
        with my_vcr.use_cassette("contacts/create.json"):
            contact = await self.client.contacts.create(
                account_id="d0ad035e-b699-4fcd-a73c-13fb0910a884",
                first_name="Test 2",
                last_name="User",
