...
async def example() -> None:
    ...
    try:
        ...
        print(
            "Parent conversion buy currency: "
            + conversions["parent_conversion"].get("buy_currency")
            + " amount "
            + conversions["parent_conversion"].get("buy_amount")
        )
        print(
            "Parent conversion sell currency: "
            + conversions["parent_conversion"].get("sell_currency")
            + " amount "
            + conversions["parent_conversion"].get("sell_amount")
        )
        print(
            "Child conversion buy currency: "
            + conversions["child_conversion"].get("sell_currency")
            + " amount "
            + conversions["child_conversion"].get("sell_amount")
        )
        print(
            "Child conversion sell currency: "
            + conversions["child_conversion"].get("buy_currency")
            + " amount "
    ...
    try:
        ...
        for element in conversions["child_conversions"]:
            ...
            print(
                "Child conversion buy currency: "
                + element.get("sell_currency")
                + " amount "
                + element.get("sell_amount")
            )
            print(
                "Child conversion sell currency: "
                + element.get("buy_currency")
                + " amount "
                + element.get("buy_amount")
            )
    ...
    try:
        ...
        for element in profit_and_loss:
            if float(element.amount) > 0:
                print(
                    f"Your profit for conversion with id {element.conversion_id}, as result of {element.event_type}, "
                    f"is {element.amount} {element.currency}"
                )
            elif float(element.amount) < 0:
                print(
                    f"Your loss for conversion with id {element.conversion_id}, as result of {element.event_type}, is "
